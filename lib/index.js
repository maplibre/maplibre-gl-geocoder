...
/**
 * @typedef {{text?:string, place_name: string, geometry?: any}} Item
 * 
 */
...
export class MaplibreGeocoder {
   ...
   * @param {[number, number, number, number]} [options.bbox] a bounding box argument: this is
  ...
  constructor(geocoderApi, options) {
    ...
    this.options = {
      ...
      clearOnBlur: false,
      getItemValue: function (/** @type Item */ item) {
        return item.text !== undefined ? item.text : item.place_name;
      },
      render: function (/** @type Item */ item) {
        // Render as a suggestion
        if (!item.geometry) {
          var suggestionString = /** @type string */ (item.text);
      ...
      popupRender: function (/** @type Item */ item) {
        var placeName = item.place_name.split(",");
        return (
   ...
   * ```
   * @param {String|HTMLElement|import('maplibre-gl').Map} container A reference to the container to which to add the geocoder
   */
  addTo(container) {
    function addToExistingContainer(/** @type MaplibreGeocoder*/ geocoder, /** @type {HTMLElement} */ container) {
    ...
    // if the container is a string, treat it as a CSS query
      ...
      addToExistingContainer(this, /** @type HTMLElement */ (parent[0]));
    } 
    // if the container is a map, add the control like normal
    else if ('_controlContainer' in container) {
  ...
  onAdd(/** @type {import('maplibre-gl').Map} */ map) {
  ...
  /**
   * @returns {{types: string[], countries: string[], language: string, bbox: number[], limit: number, proximity: number[], reverseMode: string}}
   */
  ...
  _geocode(searchInput, isSuggestion, isPlaceId) {
  ...
  _onQueryResult(response) {
    ...
    /** @type HTMLInputElement */ (this._inputEl).value = result.place_name;
    this._onChange();
  }
  ...
  _fitBoundsForMarkers() {
    ...
    if (this.options.flyTo && this._maplibregl) {
      if (this._map) {
        ...
        this._map.fitBounds(/** @type {[[number, number], [number, number]]} */ (bounds.toArray()), flyOptions);
      }
    }
   ...
   * @param {Object} geocoderApi.forwardGeocode Forward geocode constructor function should return an object including a collection of Features in [Carmen GeoJSON](https://github.com/mapbox/carmen/blob/master/carmen-geojson.md) format
   ...
   *
   * @param {Object} geocoderApi.reverseGeocode Reverse geocode constructor function should return an object including a collection of Features in [Carmen GeoJSON](https://github.com/mapbox/carmen/blob/master/carmen-geojson.md) format
  ...
  _removeResultMarkers() {
    if (this.resultMarkers && this.resultMarkers.length > 0) {
      this.resultMarkers.forEach(function (/** @type {import('maplibre-gl').Marker} */ marker) {
        marker.remove();
      });
      this.resultMarkers = [];
    }
   ...
   * @param {import('events').Listener} fn function that's called when the event is emitted.
   * @returns {MaplibreGeocoder} this;
   */
  on(type, fn) {
   ...
   * @param {import('events').Listener} fn Function that should unsubscribe to the event emitted.
   */
  off(type, fn) {
