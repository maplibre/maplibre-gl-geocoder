 ...
 * @param {Boolean} [options.searchAndSuggestWhileTyping=true] If `true` indicates that while typing `forwardGeocode` and `getSuggestions` API will be called and combined into the results list, if `false` only `getSuggestions` API will be called on typing.
 * @example
 *
 * var GeoApi = {
...
MaplibreGeocoder.prototype = {
  options: {
    ...
    debounceSearch: 200,
    searchAndSuggestWhileTyping: true,
  },
  ...
  _onKeyDown: function (e) {
    ...
    if (
      ...
      // Only call getSuggestions if searchAndSuggestWhileTyping is false
      if (
        this.geocoderApi.getSuggestions &&
        !this.options.searchAndSuggestWhileTyping
      ) {
