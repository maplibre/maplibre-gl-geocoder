...
describe("geocoder", () => {
  let map: MapMock;
  let geocoder: MaplibreGeocoder;

  function setup(opts?: Parameters<typeof init>[0]) {
  ...
  test("Query twice does for the same value does not fire result event twice", async () => {
    setup({
      proximity: { longitude: -79.45, latitude: 43.65 },
      features: [Features.QUEEN_STREET],
    });
    geocoder.query("Queen Street");
    const spy = vi.fn();
    geocoder.on("result", spy);
    await geocoder.query("Queen Street");
  ...
  test("options", async () => {
    ...
    expect(result.result.place_name).toBe("Paris, France");
  });
  ...
  test("options.reverseGeocode - false by default", async () => {
    const mockApi = createMockGeocoderApiWithSuggestions([], []);
    mockApi.reverseGeocode = vi.fn();
    setup({
      geocoderApi: mockApi
    });
    geocoder.query("-6.1933875, 34.5177548");
    await geocoder.once("results");
    expect(mockApi.reverseGeocode).not.toHaveBeenCalled();
  });
  ...
  test("options.localGeocoder", async () => {
    setup({
      ...
      limit: 6,
      localGeocoder: (q) => {
        return [{ text: q } as CarmenGeojsonFeature];
      },
    });
  ...
  test("options.render", async () => {
    setup({
      render: (feature) => {
        return "feature id is " + (feature as CarmenGeojsonFeature).id;
      },
    });

    expect(geocoder.getRenderFunction()).toBeDefined();
    expect(typeof geocoder.getRenderFunction()).toBe("function");
    expect(geocoder.getRenderFunction()({ ...Features.QUEEN_STREET, id: "abc123" })).toBe("feature id is abc123");
  });
  ...
  test("getRenderFunction", () => {
    setup({
      render: (item) => {
        return (item as CarmenGeojsonFeature).place_name;
      },
    });
    const result = geocoder.getRenderFunction();
    expect(result).toBeDefined();
  ...
  test("options.getItemValue", () => {
    const getItemValueSpy = vi.fn();
    setup({
      typeaheadFactory: () => {
        return {
          set getItemValue(value: any) {
            getItemValueSpy(value);
          }
        } as any;
      },
    ...
    expect(getItemValueSpy).toHaveBeenCalled();
  });
  ...
  test("options.marker [true]", async () => {
    ...
    setup({
      features: [Features.GOLDEN_GATE_BRIDGE],
      marker: true,
      maplibregl: { Marker: markerConstructorSpy, LngLatBounds: LngLatBoundsMock } as any
    });
  ...
  test("options.popup [true]", async () => {
    ...
    setup({
      marker: true,
      popup: true,
      maplibregl: { Popup: popupConstructorSpy, Marker: createMarkerMock(), LngLatBounds: LngLatBoundsMock } as any,
      features: [Features.GOLDEN_GATE_BRIDGE],
    });
  ...
  test("geocoder#setLanguage", () => {
    ...
    expect(geocoder.getLanguage()).toBe("en-US");
  });
  ...
  test("geocoder#setZoom", () => {
    setup({ zoom: 14 });
    expect(geocoder.getZoom()).toBe(14);
    geocoder.setZoom(17);
    expect(geocoder.getZoom()).toBe(17);
  });
  ...
  test("geocoder#setFlyTo", () => {
    ...
    expect(geocoder.getFlyTo()).toEqual({ speed: 25 });
  });
  ...
  test("geocoder#setPlaceholder", () => {
    ...
    expect(geocoder.getPlaceholder()).toBe("Search");
  });
  ...
  test("geocoder#setBbox", () => {
    ...
    expect(geocoder.getBbox()).toEqual([-2, -2, 2, 2]);
  });
  ...
  test("geocoder#setCountries", () => {
    setup({ countries: "ca" });
    expect(geocoder.getCountries()).toBe("ca");
  ...
  test("geocoder#setTypes", () => {
    ...
    expect(geocoder.getTypes()).toBe("place,poi");
  });
  ...
  test("geocoder#setLimit", () => {
    setup({ limit: 1 });
    expect(geocoder.getLimit()).toBe(1);
    geocoder.setLimit(4);
    expect(geocoder.getLimit()).toBe(4);
  });
  ...
  test("geocoder#getFilter", () => {
    ...
    expect((["a", "b", "c"] as any).filter(filter)).toEqual([]);
  });
  ...
  test("geocoder#setFilter", () => {
    ...
    const filtered = (["a", "b", "c"] as any).filter(initialFilter);
    expect(typeof initialFilter).toBe("function");
    expect(filtered).toEqual(["a", "b", "c"]);
    geocoder.setFilter(() => {
      return false;
    });
    const nextFilter = geocoder.getFilter();
    const nextFiltered = (["a", "b", "c"] as any).filter(nextFilter);
  ...
  test("query with suggestions", async () => {
    ...
    expect((e as MaplibreGeocoderSuggestionResults).suggestions).not.toBeNull();
  });
  ...
  test("set input with suggestions", async () => {
    ...
    expect((e as MaplibreGeocoderSuggestionResults).suggestions).toBeDefined();
  });
